...
use crate::world::fbx_loader::{
    AnimationClip, BoneKeyframes, EmbeddedTexture, ModelScene, Skeleton, SkinnedMesh,
};
...
pub fn load_gltf_with_animations(data: &[u8]) -> Result<ModelScene> {
    ...
    let gltf: Value = serde_json::from_str(&json_data).context("Failed to parse glTF JSON")?;
    ...
    if let Some(skins) = gltf.get("skins").and_then(|s| s.as_array()) {
        if let Some(skin) = skins.first() {
            ...
            if let Some(joints) = skin.get("joints").and_then(|j| j.as_array()) {
                ...
                let inverse_bind_matrices = if let Some(accessor_idx) =
                    skin.get("inverseBindMatrices").and_then(|i| i.as_u64())
                {
    ...
    if let Some(animations) = gltf.get("animations").and_then(|a| a.as_array()) {
        for animation in animations {
            if let Ok(clip) =
                extract_animation_manual(&gltf, animation, &buffers, &joint_to_bone_index)
            {
...
pub fn load_gltf_file(path: &str) -> Result<ModelScene> {
    let data =
        std::fs::read(path).with_context(|| format!("Failed to read glTF file: {}", path))?;
            ...
            if let Some(mesh) = meshes.get(mesh_idx as usize) {
                if let Some(primitives) = mesh.get("primitives").and_then(|p| p.as_array()) {
                    for primitive in primitives {
                        if let Ok(mut mesh_data) = extract_primitive_mesh_manual(
                            gltf,
                            primitive,
                            buffers,
                            texture_names,
                            ...
                            if has_joints && has_weights {
                                if let Ok((bone_indices, bone_weights)) =
                                    extract_skin_data_manual(gltf, primitive, buffers)
                                {
            ...
            if let Some(buffer_view_idx) = image.get("bufferView").and_then(|bv| bv.as_u64()) {
                ...
                if let Some(buffer_views) = gltf.get("bufferViews").and_then(|bv| bv.as_array()) {
                    if let Some(view) = buffer_views.get(buffer_view_idx as usize) {
                        let buffer_idx =
                            view.get("buffer").and_then(|b| b.as_u64()).unwrap_or(0) as usize;
                        let offset =
                            view.get("byteOffset").and_then(|o| o.as_u64()).unwrap_or(0) as usize;
                        let length =
                            view.get("byteLength").and_then(|l| l.as_u64()).unwrap_or(0) as usize;
...
fn extract_materials_manual(gltf: &Value) -> Vec<MaterialData> {
    ...
    if let Some(material_list) = gltf.get("materials").and_then(|m| m.as_array()) {
        for material in material_list {
            ...
            let base_color = if let Some(factor) = pbr
...
fn get_accessor_info(
    gltf: &Value,
    accessor_idx: usize,
) -> Option<(usize, usize, usize, usize, u64)> {
...
fn read_accessor_mat4_manual(
    gltf: &Value,
    accessor_idx: usize,
    buffers: &[Vec<u8>],
) -> Result<Vec<Mat4>> {
    let (buffer_idx, offset, count, stride, _) =
        get_accessor_info(gltf, accessor_idx).ok_or_else(|| anyhow!("Invalid accessor"))?;
...
fn read_accessor_indices_manual(
    gltf: &Value,
    accessor_idx: usize,
    buffers: &[Vec<u8>],
) -> Result<Vec<u32>> {
    let (buffer_idx, offset, count, stride, component_type) =
        get_accessor_info(gltf, accessor_idx).ok_or_else(|| anyhow!("Invalid accessor"))?;
    ...
    match component_type {
        5121 => {
            ...
            for i in 0..count {
                ...
                values.push([
                    buffer[start] as u32,
                    buffer[start + 1] as u32,
                    buffer[start + 2] as u32,
...
fn extract_skin_data_manual(
    gltf: &Value,
    primitive: &Value,
    buffers: &[Vec<u8>],
) -> Result<(Vec<[u32; 4]>, Vec<[f32; 4]>)> {
    let attributes = primitive
        .get("attributes")
        .ok_or_else(|| anyhow!("No attributes"))?;
...
pub fn load_animated_model(path: &str) -> Result<ModelScene> {
    let data =
        std::fs::read(path).with_context(|| format!("Failed to read animated model: {}", path))?;
...
pub fn load_animated_model_from_bytes(data: &[u8]) -> Result<ModelScene> {
    ...
    if version > VERSION {
        return Err(anyhow!(
            "SANIM version {} is newer than supported ({})",
            version,
            VERSION
        ));
    }
...
pub fn create_test_animated_model() -> ModelScene {
    ...
    clip.channels.push(BoneKeyframes {
        bone_index: 1,
        position_keys: vec![(0.0, Vec3::new(0.0, 0.5, 0.0))],
        rotation_keys: vec![
            (0.0, Quat::IDENTITY),
...
fn write_skinned_mesh(data: &mut Vec<u8>, skinned: &SkinnedMesh) {
    ...
    for (i, v) in skinned.mesh.vertices.iter().enumerate() {
        ...
        // Bone indices
        let bi = if i < skinned.bone_indices.len() {
            skinned.bone_indices[i]
        } else {
            [0; 4]
        };
        write_u32(data, bi[0]);
        ...
        write_u32(data, bi[3]);
        // Bone weights
        let bw = if i < skinned.bone_weights.len() {
            skinned.bone_weights[i]
        } else {
            [1.0, 0.0, 0.0, 0.0]
        };
        write_f32(data, bw[0]);
