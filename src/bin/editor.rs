...
use stfsc_engine::graphics::viewport_renderer::{
    ViewportMeshHandle, ViewportMeshInstance, ViewportRenderParams, ViewportRenderer,
};
...
use stfsc_engine::world::{
    ...
    scripting::BUILTIN_SCRIPT_NAMES,
    EditorEntityId, GameWorld, LightType, LocalTransform, Player, RigidBodyHandle, SceneUpdate,
    Transform, LAYER_CHARACTER, LAYER_DEFAULT, LAYER_ENVIRONMENT, LAYER_PROP, LAYER_VEHICLE,
};
...
impl Camera3D {
    ...
    fn from_eye_forward(eye: Vec3, forward: Vec3, fov: f32) -> Self {
        let forward = forward.normalize();
        let distance = 0.01;
        let target = eye.add(&forward.mul(distance));
        let pitch = (-forward.y).asin().clamp(-1.5, 1.5);
...
enum AppCommand {
    DetectQuest,
    ConnectQuestPush { device: QuestDevice },
    ConnectDebugPush { addr: String },
    DisconnectPush,
    Send(SceneUpdate),
}
enum AppEvent {
    QuestDetected(Option<QuestDevice>),
    Connected,
    Disconnected,
...
fn normalize_push_addr(addr: &str) -> String {
...
fn parse_quest_from_adb_devices(output: &str) -> Option<QuestDevice> {
    for line in output.lines().skip(1) {
        ...
        if !lower.contains("\tdevice") && !lower.contains(" device") {
            continue;
        }
        if lower.contains("offline") || lower.contains("unauthorized") {
            continue;
        }
        ...
        let model = trimmed
            ...
            .unwrap_or("ADB device")
            .replace('_', " ");

        return Some(QuestDevice { serial, model });
    }

    None
}

fn detect_quest_device() -> Option<QuestDevice> {
    ...
    parse_quest_from_adb_devices(&stdout)
}

fn forward_quest_debug_port(device: &QuestDevice) -> Result<(), String> {
    let output = std::process::Command::new("adb")
        .args(["-s", &device.serial, "forward", "tcp:8080", "tcp:8080"])
        .output()
...
fn default_script_source_for(name: &str) -> String {
    if name == CUSTOM_FUCKSCRIPT_NAME {
        ScriptComponent::custom().source
    } else {
        ScriptComponent::builtin(name).source
    }
}

fn set_script_component_preset(component: &mut ScriptComponent, new_name: &str) {
    let old_name = component.name.clone();
    if old_name == new_name {
        return;
    }

    let source_was_default = component.source.trim().is_empty()
        || component.source == default_script_source_for(&old_name);
    component.name = new_name.to_string();

    if source_was_default {
        component.source = default_script_source_for(new_name);
        component.compile_mode = if new_name == CUSTOM_FUCKSCRIPT_NAME {
            ScriptCompileMode::CustomNativeCache
        } else {
            ScriptCompileMode::BuiltinNative
        };
    } else if new_name == CUSTOM_FUCKSCRIPT_NAME {
        component.compile_mode = ScriptCompileMode::CustomNativeCache;
    } else if matches!(component.compile_mode, ScriptCompileMode::CustomNativeCache) {
...
struct EditorApp {
    ...
    push_connected: bool,
    quest_device: Option<QuestDevice>,
    scenes: Vec<String>,
    ...
    play_scene_snapshot: Option<Scene>,
    play_scene_dirty_snapshot: bool,
    play_camera_snapshot: Option<Camera3D>,
    play_player_position: Vec3,
    play_player_yaw: f32,
    play_player_pitch: f32,
    play_last_update: Option<std::time::Instant>,
    play_mouse_captured: bool,
    play_paused: bool,
    play_jump_was_down: bool,
    play_runtime: Option<EmbeddedPlayRuntime>,
    ...
    ui_drag_last_delta: egui::Vec2, // Last cumulative drag delta applied in UI canvas
...
impl EditorApp {
    fn play_forward(&self) -> Vec3 {
        ...
        Vec3::new(
            self.play_player_yaw.sin() * pitch_cos,
            self.play_player_pitch.sin(),
            -self.play_player_yaw.cos() * pitch_cos,
        )
        .normalize()
    }

    fn play_angles_from_forward(forward: Vec3) -> (f32, f32) {
        let forward = forward.normalize();
        let yaw = forward.x.atan2(-forward.z);
        let pitch = forward.y.asin().clamp(-1.45, 1.45);
        (yaw, pitch)
    }

    fn player_start_pose(&self) -> (Vec3, f32, f32, f32) {
        let mut position = Vec3::new(0.0, 1.7, 0.0);
        let mut forward = Vec3::new(0.0, 0.0, -1.0);
        let mut fov = 75.0;

        if let Some(scene) = &self.current_scene {
            if let Some(player_start) = scene
                .entities
                .iter()
                .find(|entity| matches!(entity.entity_type, EntityType::Camera))
            {
                position = Vec3::new(
                    player_start.position[0],
                    player_start.position[1],
                    player_start.position[2],
                );

                let mut rotation = GQuat::from_array(player_start.rotation);
                rotation = if rotation.length_squared() > 0.0001 {
                    rotation.normalize()
                } else {
                    GQuat::IDENTITY
                };
                let rotation_forward = rotation * GVec3::NEG_Z;
                if rotation_forward.length_squared() > 0.0001 {
                    forward = Vec3::new(rotation_forward.x, rotation_forward.y, rotation_forward.z);
                }

                if player_start.fov > 0.0 {
                    fov = if player_start.fov <= std::f32::consts::PI {
                        player_start.fov.to_degrees()
                    } else {
                        player_start.fov
                    };
                }
        ...
        let (yaw, pitch) = Self::play_angles_from_forward(forward);
        (position, yaw, pitch, fov)
    }

    fn set_play_mouse_capture(&mut self, ctx: &egui::Context, captured: bool) {
        if self.play_mouse_captured == captured {
            return;
        }

        self.play_mouse_captured = captured;
    ...
    fn queue_embedded_runtime_update(
        world: &mut GameWorld,
        physics: &mut PhysicsWorld,
        player_pos: GVec3,
        update: SceneUpdate,
    ) {
        match world.command_sender.try_send(update) {
            Ok(()) => {}
            Err(tokio::sync::mpsc::error::TrySendError::Full(update)) => {
                world.update_streaming(player_pos, physics);
                let _ = world.command_sender.try_send(update);
            }
    ...
    fn scene_runtime_updates(&self, scene: &Scene) -> Vec<SceneUpdate> {
        let mut updates = vec![
            SceneUpdate::SetSandboxSettings {
                settings: scene.sandbox.clone(),
            },
            SceneUpdate::SetProceduralGeneration {
                enabled: self.procedural_generation_enabled || scene.sandbox.enabled,
            },
        ...
        for named_layout in &scene.ui_layouts {
            let layer = match named_layout.layer_type {
                ...
                _ => stfsc_engine::ui::UiLayer::Custom(named_layout.alias.clone()),
            };
            let mut layout = named_layout.layout.clone();
            layout.layer_type = named_layout.layer_type;
            updates.push(SceneUpdate::SetUiLayer { layer, layout });
        }
    ...
    fn create_embedded_play_runtime(&self, eye_position: Vec3) -> EmbeddedPlayRuntime {
        ...
        let player_rb = physics.add_capsule_rigid_body(
            0,
            body_position,
            0.6,
            0.3,
        ...
        if let Some(body) = physics.rigid_body_set.get_mut(player_rb) {
            body.set_locked_axes(rapier3d::prelude::LockedAxes::ROTATION_LOCKED, true);
            body.set_linear_damping(0.0);
        }

        let player_position = GVec3::new(eye_position.x, eye_position.y, eye_position.z);
        let player_entity = world.ecs.spawn((
            Transform {
                position: player_position,
                rotation: GQuat::from_rotation_y(self.play_player_yaw),
                scale: GVec3::ONE,
            },
            LocalTransform {
                position: player_position,
        ...
        EmbeddedPlayRuntime {
            ...
            world,
            player_entity,
            player_rb: RigidBodyHandle(player_rb),
        }
    }

    fn sync_embedded_runtime_transforms(&mut self) {
        ...
        let updates: Vec<_> = runtime
            .world
            .ecs
            .query::<(&EditorEntityId, &Transform)>()
            .iter()
            .map(|(_, (editor_id, transform))| {
                (
                    editor_id.0,
    ...
    fn update_embedded_play_camera(&mut self, ui: &mut egui::Ui, response: &egui::Response) {
        ...
        let dt = self
            ...
            .unwrap_or(0.0)
            .clamp(0.0, 0.1);
        self.play_last_update = Some(now);

        let can_drive = self.play_mouse_captured && !self.play_paused;
        let (move_forward, move_backward, move_left, move_right, sprint, jump_down, look_delta) =
            ui.input(|i| {
                (
                    can_drive && i.key_down(egui::Key::W),
                    can_drive && i.key_down(egui::Key::S),
                    can_drive && i.key_down(egui::Key::A),
                    can_drive && i.key_down(egui::Key::D),
                    can_drive && i.modifiers.shift,
                    can_drive && i.key_down(egui::Key::Space),
                    if can_drive {
                        i.pointer.motion().unwrap_or(egui::Vec2::ZERO)
                    } else {
                        egui::Vec2::ZERO
                    },
                )
        ...
        if look_delta.x != 0.0 || look_delta.y != 0.0 {
            ...
            self.play_player_yaw += look_delta.x * sensitivity;
            self.play_player_pitch =
                (self.play_player_pitch - look_delta.y * sensitivity).clamp(-1.45, 1.45);
        }

        let jump_pressed = jump_down && !self.play_jump_was_down;
        ...
        if let Some(runtime) = &mut self.play_runtime {
            if can_drive {
                ...
                if move_forward {
                    move_dir += forward_basis;
                }
                if move_backward {
                    move_dir -= forward_basis;
                }
                if move_left {
                    move_dir -= right_basis;
                }
                if move_right {
                    move_dir += right_basis;
                }

                if let Some(body) = runtime.physics.rigid_body_set.get_mut(runtime.player_rb.0) {
                    let current_vel = *body.linvel();
                    let target_vel = if move_dir.length_squared() > 0.0 {
                        move_dir.normalize() * if sprint { 12.0 } else { 5.0 }
                    } else {
                        GVec3::ZERO
                    };
                    let mut new_linvel =
                        rapier3d::na::Vector3::new(target_vel.x, current_vel.y, target_vel.z);
                    if jump_pressed && current_vel.y.abs() < 0.1 {
                        new_linvel.y = 5.0;
                    }
            ...
            if can_drive {
                runtime.physics.step_with_dt(dt);
            }

            if let Some(body) = runtime.physics.rigid_body_set.get(runtime.player_rb.0) {
                let phys_pos = *body.translation();
                let eye = GVec3::new(phys_pos.x, phys_pos.y, phys_pos.z) + GVec3::Y * 0.8;
                runtime_player_position = Some(eye);
            }
            ...
            if let Some(player_pos) = runtime_player_position {
                ...
                if let Ok(mut local) = runtime
                    .world
                    .ecs
                    .get::<&mut LocalTransform>(runtime.player_entity)
                {
                    local.position = player_pos;
                ...
                xr_snapshot.left.grip_pose = stfsc_engine::world::scripting::XrPose::new(
                    player_pos + head_rot * GVec3::new(-0.28, -0.28, -0.45),
                    head_rot,
                    true,
                );
                xr_snapshot.left.aim_pose = xr_snapshot.left.grip_pose;
                xr_snapshot.left.thumbstick = {
                ...
                xr_snapshot.right.grip_pose = stfsc_engine::world::scripting::XrPose::new(
                    player_pos + head_rot * GVec3::new(0.28, -0.28, -0.45),
                    head_rot,
                    true,
                );
                xr_snapshot.right.aim_pose = xr_snapshot.right.grip_pose;
                xr_snapshot.right.primary_button = jump_down;
                xr_snapshot.right.secondary_button = sprint;
                xr_snapshot.set_action_pressed("jump", jump_down);
                xr_snapshot.set_action_pressed("sprint", sprint);
                ...
                let physics_updates: Vec<_> = runtime
                    ...
                    .filter_map(|(entity, (_, handle))| {
                        runtime.physics.rigid_body_set.get(handle.0).map(|body| {
                            let translation = *body.translation();
                            let rotation = *body.rotation();
                            (
                                entity,
                                GVec3::new(translation.x, translation.y, translation.z),
                                GQuat::from_xyzw(rotation.i, rotation.j, rotation.k, rotation.w),
                            )
                        })
                    })
                    .collect();
                for (entity, position, rotation) in physics_updates {
                    if let Ok(mut transform) = runtime.world.ecs.get::<&mut Transform>(entity) {
                        transform.position = position;
                        transform.rotation = rotation;
                    }
                    if let Ok(mut local) = runtime.world.ecs.get::<&mut LocalTransform>(entity) {
        ...
        if let Some(player_pos) = runtime_player_position {
            self.play_player_position = Vec3::new(player_pos.x, player_pos.y, player_pos.z);
        }
        self.sync_embedded_runtime_transforms();
        self.camera = Camera3D::from_eye_forward(
            self.play_player_position,
            self.play_forward(),
            self.camera.fov,
        );
    }

    fn enter_play_mode(&mut self, ctx: &egui::Context) {
        if self.play_mode {
            return;
        }
        ...
        let now = std::time::Instant::now();
        self.play_started_at = Some(now);
        self.play_last_update = Some(now);

        let (position, yaw, pitch, fov) = self.player_start_pose();
        self.play_player_position = position;
    ...
    fn exit_play_mode(&mut self, ctx: &egui::Context) {
        ...
        self.set_play_mouse_capture(ctx, false);
        if let Some(scene) = self.play_scene_snapshot.take() {
            self.current_scene = Some(scene);
        }
        if let Some(camera) = self.play_camera_snapshot.take() {
            self.camera = camera;
        }
        self.scene_dirty = self.play_scene_dirty_snapshot;
        self.play_started_at = None;
        self.play_last_update = None;
        self.play_runtime = None;
        self.play_paused = false;
        self.play_jump_was_down = false;
        self.play_mode = false;
        self.status = "Embedded Game View stopped".into();
    }

    fn toggle_play_mode(&mut self, ctx: &egui::Context) {
        if self.play_mode {
    ...
    fn new() -> Self {
        ...
        thread::spawn(move || {
            ...
            while let Ok(cmd) = command_rx.recv() {
                match cmd {
                    AppCommand::DetectQuest => {
                        let detected = detect_quest_device();
                        let _ = event_tx.send(AppEvent::QuestDetected(detected));
                    }
                    AppCommand::ConnectQuestPush { device } => {
                        let _ = event_tx.send(AppEvent::StatusUpdate("Connecting push...".into()));
                        if let Err(e) = forward_quest_debug_port(&device) {
                            let _ = event_tx.send(AppEvent::ConnectionError(e));
                            continue;
                        }
                        ...
                        let socket_addr = match addr.parse() {
                            Ok(addr) => addr,
                            Err(e) => {
                                let _ = event_tx.send(AppEvent::ConnectionError(format!(
                    ...
                    AppCommand::ConnectDebugPush { addr } => {
                        let _ = event_tx.send(AppEvent::StatusUpdate("Connecting push...".into()));
                        let addr = normalize_push_addr(&addr);
                    ...
                    AppCommand::Send(update) => {
                        if let Some(s) = &mut stream {
                            ...
                            if s.write_all(&len.to_le_bytes()).is_ok()
                                && s.write_all(&bytes).is_ok()
                            {
                                ...
                                stream = None;
                                let _ = event_tx
                                    .send(AppEvent::ConnectionError("Connection lost".into()));
                            }
                        } else {
        ...
        Self {
            push_addr: "127.0.0.1:8080".into(),
            status: "Embedded player ready".into(),
            command_tx,
            event_rx,
            ...
            play_scene_snapshot: None,
            play_scene_dirty_snapshot: false,
            play_camera_snapshot: None,
            play_player_position: Vec3::new(0.0, 1.7, 0.0),
            play_player_yaw: 0.0,
            play_player_pitch: 0.0,
            play_last_update: None,
            play_mouse_captured: false,
            play_paused: false,
            play_jump_was_down: false,
            play_runtime: None,
            camera: Camera3D::new(),
            drag_button: None,
            ...
            ui_drag_last_delta: egui::Vec2::ZERO,
            ...
            model_cache: {
                ...
                for ptype in PrimitiveType::all() {
                    ...
                    cache.insert(
                        format!("primitive://{}", ptype.name().to_lowercase()),
                        Arc::new(scene),
                    );
                }
                cache
            ...
            viewport_graphics: {
                ...
                match GraphicsContext::new_headless() {
                    ...
                    Err(e) => {
                        println!(
                            "GPU Viewport: Vulkan init failed ({}), using software fallback",
                            e
                        );
                        None
                    }
    ...
    fn push_entity_to_live_runtime(&self, entity: &SceneEntity) {
        match &entity.entity_type {
            ...
            EntityType::Light {
                ...
                let pos = (glam::Vec3::from(entity.position)
                    + glam::Vec3::new(epsilon_x, epsilon_y, epsilon_z))
                .to_array();
            ...
            _ => {
                ...
                if let (Some(texture_id), Some(texture_data)) = (
                    ...
                    let _ = self
                        .command_tx
                        .send(AppCommand::Send(SceneUpdate::UploadTexture {
                            id: texture_id.clone(),
                            data: texture_data.clone(),
                        }));
                }
                ...
                if matches!(&entity.entity_type, EntityType::Ground) {
                    let _ = self
                        .command_tx
                        .send(AppCommand::Send(SceneUpdate::SpawnGroundPlane {
                            id: entity.id,
                            primitive: 0, // Cube
                            position: entity.position,
                            scale: entity.scale,
        ...
        if !script_names.is_empty() {
            let _ = self
                .command_tx
                .send(AppCommand::Send(SceneUpdate::SetScripts {
                    id: entity.id,
                    names: script_names,
                }));
        }
    ...
    fn push_scene_to_live_runtime(&self) {
        if let Some(scene) = &self.current_scene {
            ...
            // Deploy all entities
            for entity in &scene.entities {
                self.push_entity_to_live_runtime(entity);
            }

            // Deploy all UI layouts
            for nl in &scene.ui_layouts {
                let layer = match nl.layer_type {
                    stfsc_engine::ui::UiLayerType::PauseOverlay => {
                        stfsc_engine::ui::UiLayer::PauseMenu
                    }
                    stfsc_engine::ui::UiLayerType::MainMenu => stfsc_engine::ui::UiLayer::MainMenu,
                    stfsc_engine::ui::UiLayerType::InGameOverlay => stfsc_engine::ui::UiLayer::Hud,
                ...
                let _ = self
                    .command_tx
                    .send(AppCommand::Send(SceneUpdate::SetUiLayer {
                        layer,
    ...
    fn add_primitive(&mut self, ptype: PrimitiveType) {
        if let Some(scene) = &mut self.current_scene {
            ...
            // Push to undo stack
            self.undo_stack.push(EditorAction::AddEntity {
    ...
    fn load_scene_from_path(&mut self, ctx: &egui::Context, path: &str) -> Result<(), String> {
        ...
        let contents =
            std::fs::read_to_string(&absolute_path).map_err(|e| format!("Read error: {}", e))?;
            ...
            if let Some(texture_path) = entity.material.albedo_texture.clone() {
                ...
                if let Some(proj) = &self.current_project {
                    paths_to_try.push(
                        proj.root_path
                            ...
                            .join(std::path::Path::new(&texture_path).file_name().unwrap())
                            .to_string_lossy()
                            .to_string(),
                    );
                }
                ...
                if let Some(p) = loaded_path {
                    ...
                    if let Some(data) = &entity.material.albedo_texture_data {
                        let tex_name =
                            std::path::Path::new(entity.material.albedo_texture.as_ref().unwrap())
                                .file_name()
                                .unwrap_or_default()
                                .to_string_lossy()
                                .to_string();
    ...
    fn cache_model_at_path(
        &mut self,
        ctx: &egui::Context,
        path: &str,
    ) -> Result<Arc<ModelScene>, String> {
    ...
    fn import_model_from_path(&mut self, ctx: &egui::Context, path: &str) -> Result<(), String> {
        ...
        if self.push_connected {
            let _ = self
                .command_tx
                .send(AppCommand::Send(SceneUpdate::SpawnFbxMesh {
                    id: new_entity.id,
                    mesh_data: data,
            ...
            if let Some(ref anim_config) = new_entity.animator_config {
                let _ = self
                    .command_tx
                    .send(AppCommand::Send(SceneUpdate::AttachAnimator {
                        id: new_entity.id,
    ...
    fn deploy_mesh_entity(&self, entity: &SceneEntity, mesh_data: &[u8]) {
        ...
        if is_gltf {
            let _ = self
                .command_tx
                .send(AppCommand::Send(SceneUpdate::SpawnGltfMesh {
                    id: entity.id,
    ...
    fn load_egui_texture(&mut self, ctx: &egui::Context, name: &str, data: &[u8]) {
        ...
        if let Ok(img) = image::load_from_memory(data) {
            ...
            let handle = ctx.load_texture(name, color_img, egui::TextureOptions::LINEAR);
            ...
            match &action {
                    ...
                    if let Some(scene) = &mut self.current_scene {
                        if let Some(nl) = scene.ui_layouts.get_mut(*layout_idx) {
                            match element_type.as_str() {
                                "Button" => {
                                    nl.layout.buttons.pop();
                                }
                                "Panel" => {
                                    nl.layout.panels.pop();
                                }
                                "Text" => {
                                    nl.layout.texts.pop();
                                }
                ...
                EditorAction::DeleteUiElement {
                    ...
                    if let Some(scene) = &mut self.current_scene {
                        if let Some(nl) = scene.ui_layouts.get_mut(*layout_idx) {
                            match element {
                                UiElementHistory::Button(b) => {
                                    nl.layout.buttons.insert(*element_idx, b.clone())
                                }
                                UiElementHistory::Panel(p) => {
                                    nl.layout.panels.insert(*element_idx, p.clone())
                                }
                                UiElementHistory::Text(t) => {
                                    nl.layout.texts.insert(*element_idx, t.clone())
                                }
                            }
                        }
                    }
                ...
                EditorAction::ModifyUiElement {
                    ...
                    if let Some(scene) = &mut self.current_scene {
                        if let Some(nl) = scene.ui_layouts.get_mut(*layout_idx) {
                            match before {
                                UiElementHistory::Button(b) => {
                                    if let Some(target) = nl.layout.buttons.get_mut(*element_idx) {
                                        *target = b.clone();
                                    }
                                }
                                UiElementHistory::Panel(p) => {
                                    if let Some(target) = nl.layout.panels.get_mut(*element_idx) {
                                        *target = p.clone();
                                    }
                                }
                                UiElementHistory::Text(t) => {
                                    if let Some(target) = nl.layout.texts.get_mut(*element_idx) {
                                        *target = t.clone();
                                    }
                                }
                            }
                ...
                EditorAction::DeleteUiElement {
                    ...
                    if let Some(scene) = &mut self.current_scene {
                        if let Some(nl) = scene.ui_layouts.get_mut(*layout_idx) {
                            match element_type.as_str() {
                                "Button" => {
                                    nl.layout.buttons.remove(*element_idx);
                                }
                                "Panel" | "PanelChild" => {
                                    nl.layout.panels.remove(*element_idx);
                                }
                                "Text" => {
                                    nl.layout.texts.remove(*element_idx);
                                }
                                _ => {}
                            }
                        }
    ...
    fn try_gpu_render(
        ...
        // Fast when the viewport is being manipulated, much slower when other UI owns input.
        ...
        let effective_interval = if scene_interacting {
            self.gpu_frame_interval_ms // Active viewport: 16ms (60 FPS)
        } else if self.viewport_idle_frames < 10 {
            33 // Brief settle period after movement
        } else {
            250 // Idle/UI navigation: avoid readback spikes while menus and panels are in use
        };
            ...
            if !self.gpu_mesh_cache.contains_key(path) {
                ...
                for mesh in &model.meshes {
                    match renderer.upload_mesh(mesh) {
                        ...
                        Err(e) => {
                            println!("GPU Mesh: Failed to upload submesh in '{}': {}", path, e)
                        }
                    }
                }
                ...
                for skinned in &model.skinned_meshes {
                    match renderer.upload_mesh(&skinned.mesh) {
                        ...
                        Err(e) => println!(
                            "GPU Mesh: Failed to upload skinned submesh in '{}': {}",
                            path, e
                        ),
                    }
                }
                ...
                if !handles.static_meshes.is_empty() || !handles.skinned_meshes.is_empty() {
                    println!(
                        "GPU Model: Uploaded '{}' ({} static, {} skinned)",
                        path,
                        handles.static_meshes.len(),
                        handles.skinned_meshes.len()
                    );
        ...
        let light_view_proj =
            shadow_proj * glam::Mat4::look_at_rh(light_pos, shadow_center, glam::Vec3::Y);
        ...
        if let Some(scene) = &self.current_scene {
            for entity in &scene.entities {
                let mesh_path = match &entity.entity_type {
                    ...
                    EntityType::Primitive(ptype) => {
                        Some(format!("primitive://{}", ptype.name().to_lowercase()))
                    }
                ...
                if let Some(path) = mesh_path {
                    if let Some(model_handles) = self.gpu_mesh_cache.get(&path) {
                        ...
                        let joints = if entity.animator_config.is_some() {
                            EditorApp::compute_animated_pose(
                                entity,
                                &self.model_cache,
                                &self.animation_editor_state,
                            )
                            .map(|(_, skin)| skin)
                        } else {
                            None
                        ...
                        let material_descriptor_set = if let Some(tex_name) =
                            &entity.material.albedo_texture
                        {
                                ...
                                if let Some(tex_data) = &entity.material.albedo_texture_data {
                                    match graphics.create_texture_from_bytes(tex_data) {
                                        Ok(albedo) => {
                                            ...
                                            {
                                                match graphics.create_material_descriptor_set(
                                                    &albedo, def_normal, def_mr,
                                                ) {
                                                    Ok(ds) => {
                                                        self.gpu_texture_cache
                                                            .insert(tex_name.clone(), albedo);
                                                        self.gpu_material_cache
                                                            .insert(tex_name.clone(), ds);
                                                        Some(ds)
                                                    }
                                                    Err(e) => {
                                        ...
                                        Err(e) => {
                                            println!(
                                                "GPU Viewport: Failed to upload texture '{}': {}",
                                                tex_name, e
                                            );
                                            None
                                        }
        ...
        static PRINTED_QUEUE_STATS: std::sync::atomic::AtomicBool =
            std::sync::atomic::AtomicBool::new(false);
        if !PRINTED_QUEUE_STATS.swap(true, std::sync::atomic::Ordering::Relaxed) {
            if let Some(scene) = &self.current_scene {
                println!(
                    "GPU Queue: {} entities in scene, {} queued for render",
                    scene.entities.len(),
                    renderer.pending_instance_count()
                );
            }
        }
        ...
        match renderer.render_and_readback() {
            Ok(pixels) => {
                ...
                static DEBUG_ONCE: std::sync::atomic::AtomicBool =
                    std::sync::atomic::AtomicBool::new(false);
                ...
                if let Some(existing) = &self.viewport_texture {
                    ...
                    // Create new texture
                    let handle =
                        ctx.load_texture("gpu_viewport", image, egui::TextureOptions::LINEAR);
    ...
    fn draw_3d_viewport(&mut self, ui: &mut egui::Ui) -> egui::Rect {
        ...
        if self.play_mode {
            ...
            if response.drag_stopped() {
                ...
                if let (Some(start_entity), Some(drag_id)) =
                    (self.drag_start_entity.take(), self.dragging_id)
                {
                    if let Some(scene) = &self.current_scene {
            ...
            if response.dragged() {
                ...
                if let Some(drag_id) = self.dragging_id {
                    ...
                    let (cam_pos, ray_dir) =
                        self.camera.get_ray(egui::pos2(rel_x, rel_y), rect_size);
        ...
        let gpu_rendered = if self.render_quality == RenderQuality::High {
            if let Some(texture) =
                self.try_gpu_render(ui.ctx(), rect, available, viewport_interacting)
            {
        ...
        if !gpu_rendered && !self.play_mode {
            if self.render_quality == RenderQuality::High {
                ...
                for i in 0..steps {
                    ...
                    let color = egui::Color32::from_rgb(
                        (bg_color_top.r() as f32 * (1.0 - t) + bg_color_bottom.r() as f32 * t)
                            as u8,
                        (bg_color_top.g() as f32 * (1.0 - t) + bg_color_bottom.g() as f32 * t)
                            as u8,
                        (bg_color_top.b() as f32 * (1.0 - t) + bg_color_bottom.b() as f32 * t)
                            as u8,
                    );
                    painter.rect_filled(r, 0.0, color);
                }
            ...
            if self.render_quality == RenderQuality::High {
                for i in -100..=100 {
                    ...
                    if let (Some(p1), Some(p2)) = (
                        self.camera
                            .project(Vec3::new(val, 0.0, -grid_size), available),
                        self.camera
                            .project(Vec3::new(val, 0.0, grid_size), available),
                    ) {
                    ...
                    if let (Some(p1), Some(p2)) = (
                        self.camera
                            .project(Vec3::new(-grid_size, 0.0, val), available),
                        self.camera
                            .project(Vec3::new(grid_size, 0.0, val), available),
                    ) {
                        painter.line_segment(
            ...
            if let Some(o) = self.camera.project(origin, available) {
                ...
                for (axis, color) in [
                    ...
                    (
                        ...
                        egui::Color32::from_rgb(60, 255, 60),
                    ),
                    (
                        Vec3::new(0.0, 0.0, 5.0),
                        egui::Color32::from_rgb(60, 60, 255),
                    ),
                ] {
        ...
        if let Some(scene) = &self.current_scene {
            ...
            let mut entity_renders: Vec<_> = scene
                .entities
                .par_iter()
                .map(|entity| {
                    ...
                    let _is_double_sided = matches!(
                        &entity.entity_type,
                        EntityType::Ground | EntityType::Primitive(PrimitiveType::Plane)
                    );
                    ...
                    let center_proj = camera
                        .project(pos, available)
                    ...
                    let entity_aabb = if let Some(path) = &mesh_path {
                        if let Some(model) = model_cache.get(path) {
                            ...
                            let world_center = entity_pos
                                + GQuat::from_array(entity.rotation) * (center * entity_scale);
                    ...
                    if render_quality == RenderQuality::High && is_visible && !skip_cpu_rendering {
                        ...
                        if let Some(ref path) = mesh_path {
                            if let Some(model) = model_cache.get(path) {
                                ...
                                let all_meshes: Vec<&stfsc_engine::world::Mesh> = model
                                    .meshes
                                    .iter()
                                ...
                                for mesh in all_meshes {
                                    ...
                                    let processed_verts: Vec<ProcessedVertex> = mesh
                                        .vertices
                                        .par_iter()
                                        .map(|v| {
                                            // Transform
                                            let (world_pos, world_normal) =
                                                if let Some((_, skinning_matrices)) = &pose {
                                                    ...
                                                    for i in 0..4 {
                                                        ...
                                                        if weight > 0.0 {
                                                            let bone_idx =
                                                                v.bone_indices[i] as usize;
                                                            if let Some(mat) =
                                                                skinning_matrices.get(bone_idx)
                                                            {
                                                                skinned_pos +=
                                                                    mat.transform_point3(
                                                                        GVec3::from(v.position),
                                                                    ) * weight;
                                                                skinned_norm +=
                                                                    mat.transform_vector3(
                                                                        GVec3::from(v.normal),
                                                                    ) * weight;
                                                            }
                                                        }
                                                    ...
                                                    (
                                                        transform.transform_point3(skinned_pos),
                                                        transform
                                                            .transform_vector3(skinned_norm)
                                                            .normalize(),
                                                    )
                                                } else {
                                                    (
                                                        transform.transform_point3(GVec3::from(
                                                            v.position,
                                                        )),
                                                        transform
                                                            .transform_vector3(GVec3::from(
                                                                v.normal,
                                                            ))
                                                            .normalize(),
                                                    )
                                            ...
                                            let (proj_pos, proj_valid) = camera.project_unclamped(
                                                Vec3::new(world_pos.x, world_pos.y, world_pos.z),
                                                available,
                                            );
                                            let screen_pos = egui::pos2(
                                                rect.min.x + proj_pos.x,
                                                rect.min.y + proj_pos.y,
                                            );
                                    ...
                                    let valid_tris: Vec<(usize, [usize; 3], f32)> = (0..num_tris)
                                        .into_par_iter()
                                        .filter_map(|tri_idx| {
                                            let base = tri_idx * 3;
                                    ...
                                    valid_tris.sort_by(|a, b| match b.2.partial_cmp(&a.2) {
                                    ...
                                    let mesh_avg_depth = if !valid_tris.is_empty() {
                                        valid_tris.iter().map(|(_, _, d)| *d).sum::<f32>()
                                            / valid_tris.len() as f32
                                    } else {
                                        0.0
                                    };

                                    let mut egui_vertices =
                                        Vec::with_capacity(valid_tris.len() * 3);
                                    let mut egui_indices = Vec::with_capacity(valid_tris.len() * 3);
                                    let has_texture = mesh.albedo_texture.is_some()
                                        || entity.material.albedo_texture.is_some();
                                    ...
                                    if !egui_indices.is_empty() {
                                        high_res_meshes.push((
                                            egui_vertices,
                                            egui_indices,
                                            mesh.albedo_texture.clone(),
                                            mesh_avg_depth,
                                        ));
                                    }
                                }
                    ...
                    (
                        ...
                        entity.material.albedo_texture.clone(),
                        is_light,
                        is_camera,
                    )
                })
            ...
            entity_renders
                .sort_by(|a, b| b.6.partial_cmp(&a.6).unwrap_or(std::cmp::Ordering::Equal));

            // Draw high-quality meshes (Always as fallback/overlay for now)
            let use_gpu_viewport_rendering = self.render_quality == RenderQuality::High
                && self.use_gpu_viewport
                && self.viewport_texture.is_some();
            ...
            ) in entity_renders
            {
                ...
                if !gpu_rendered && self.render_quality == RenderQuality::High && has_mesh {
                    ...
                    if shadow_dist > 0.0 && shadow_dist < 50.0 && shadow_size > 0.1 {
                        let shadow_alpha =
                            (60.0 / (shadow_dist * 0.5 + 1.0)).clamp(5.0, 40.0) as u8;
                        ...
                        if let Some(sp) = camera.project(shadow_pos, available) {
                            ...
                            if let (Some(px), Some(pz)) = (
                                camera.project(extent_x, available),
                                camera.project(extent_z, available),
                            ) {
                                ...
                                for i in 0..2 {
                                    ...
                                    let s_color =
                                        egui::Color32::from_rgba_unmultiplied(5, 10, 15, alpha);
                ...
                if !self.play_mode {
                    ...
                    if let Some(c) = center_proj {
                        ...
                        if is_light {
                            ...
                            if is_selected {
                                painter.circle_stroke(
                                    c,
                                    size + 4.0,
                                    egui::Stroke::new(2.5, egui::Color32::GOLD),
                                );
                            }
                        } else if is_camera {
                        ...
                        } else if self.render_quality == RenderQuality::Low
                            || !use_gpu_viewport_rendering
                            || !has_mesh
                        {
        ...
        if !self.play_mode && self.active_view == EditorView::AnimationEditor {
            if let Some(id) = self.selected_entity_id {
                if let Some(scene) = &self.current_scene {
                    if let Some(entity) = scene.entities.iter().find(|e| e.id == id) {
            ...
            if let Some(selected_id) = self.selected_entity_id {
                if let Some(scene) = &mut self.current_scene {
                    if let Some(entity_idx) =
                        scene.entities.iter().position(|e| e.id == selected_id)
                    {
                        ...
                        for (i, (dir, color, _label)) in axes.iter().enumerate() {
                            ...
                            if let (Some(p1), Some(p2)) = (
                                self.camera.project(start, available),
                                self.camera.project(end, available),
                            ) {
                                ...
                                if let Some(m) = mouse_pos {
                                    let dist_to_segment = {
                                        ...
                                        if l2 == 0.0 {
                                            ...
                                            let t = ((m.x - p1.x) * (p2.x - p1.x)
                                                + (m.y - p1.y) * (p2.y - p1.y))
                                                / l2;
                                            let t = t.clamp(0.0, 1.0);
                                            m.distance(egui::pos2(
                                                p1.x + t * (p2.x - p1.x),
                                                p1.y + t * (p2.y - p1.y),
                                            ))
                                        }
                                    ...
                                    if dist_to_segment < 10.0 {
                                        hovered_axis = Some(i);
                                        if !is_dragging {
                                            draw_color = egui::Color32::YELLOW;
                                        }
                                    }
                                }
                                ...
                                painter.line_segment(
                                    [p1, p2],
                                    egui::Stroke::new(
                                        if is_dragging || hovered_axis == Some(i) {
                                            4.0
                                        } else {
                                            2.5
                                        },
                                ...
                                if let Some(cone_model) = self.model_cache.get("primitive://cone") {
                                    let cone_transform = Mat4::from_scale_rotation_translation(
                                        ...
                                        GQuat::from_rotation_arc(
                                            GVec3::Y,
                                            GVec3::new(dir.x, dir.y, dir.z),
                                        ),
                                    ...
                                    for mesh in &cone_model.meshes {
                                        ...
                                        for v in &mesh.vertices {
                                            let world_v = cone_transform
                                                .transform_point3(GVec3::from(v.position));
                                            if let Some(p) = self.camera.project(
                                                Vec3::new(world_v.x, world_v.y, world_v.z),
                                                available,
                                            ) {
                        ...
                        if let Some(axis_idx) = self.gizmo_axis {
                            ...
                            if let (Some(p1), Some(p2)) = (
                                self.camera.project(entity_pos, available),
                                self.camera.project(entity_pos.add(&axis_dir), available),
                            ) {
                                ...
                                if screen_len > 0.0 {
                                    ...
                                    let projection = mouse_delta.x * normalized_screen_dir.x
                                        + mouse_delta.y * normalized_screen_dir.y;
        ...
        if !self.play_mode {
            if let Some(id) = clicked_entity {
                if self.gizmo_axis.is_none() {
                    // Don't change selection while dragging gizmo
                    self.selected_entity_id = Some(id);
                }
            }
        }
        ...
        if self.play_mode {
            painter.rect_stroke(
                rect.shrink(1.0),
                0.0,
                egui::Stroke::new(2.0, egui::Color32::from_rgb(80, 210, 120)),
            );
            let elapsed = self
                .play_started_at
            ...
            painter.text(
                ...
                format!(
                    ...
                    if self.play_paused {
                        "PAUSED"
                    } else if self.play_mouse_captured {
                        "CAPTURED"
                    } else {
                        "CLICK TO CAPTURE"
                    }
                ...
                egui::Color32::from_rgb(150, 255, 170),
            );
            painter.text(
                rect.min + egui::vec2(10.0, 30.0),
                egui::Align2::LEFT_TOP,
                "WASD move | Mouse look | Space jump | Shift sprint | Esc release/pause",
    ...
    fn draw_skeleton(
        ...
        if let Some(config) = &entity.animator_config {
            if config.bone_transforms.is_empty() {
                return;
            }

            // Try to get animated pose first
            let animated_pose = Self::compute_animated_pose(
                entity,
                &self.model_cache,
                &self.animation_editor_state,
            );
                    ...
                    if let Some(parent_idx) = config.bone_parents[i] {
                        if parent_idx < globals.len() {
                            ...
                            if let Some(p2) = camera.project(v3_p_pos, available) {
                                ...
                                painter.line_segment(
                                    [p1_egui, p2_egui],
                                    egui::Stroke::new(1.5, bone_color),
                                );
                            }
                        }
                    }
    ...
    fn draw_keyframe_inspector(&mut self, ui: &mut egui::Ui) {
        ...
        if state.selected_keyframes.is_empty() {
            ui.vertical_centered(|ui| {
                ...
                ui.label(
                    egui::RichText::new("Click a keyframe in the timeline to edit its properties")
                        .small(),
                );
            });
            return;
                        ...
                        match &mut kf.value {
                            ...
                            KeyframeValue::Quat(q) => {
                                egui::Grid::new("quat_grid").show(ui, |ui| {
                                    ui.label("X");
                                    if ui
                                        .add(egui::DragValue::new(&mut q[0]).speed(0.02))
                                        .changed()
                                    {
                                        self.scene_dirty = true;
                                    }
                                    ui.label("Y");
                                    if ui
                                        .add(egui::DragValue::new(&mut q[1]).speed(0.02))
                                        .changed()
                                    {
                                        self.scene_dirty = true;
                                    }
                                    ...
                                    ui.label("Z");
                                    if ui
                                        .add(egui::DragValue::new(&mut q[2]).speed(0.02))
                                        .changed()
                                    {
                                        self.scene_dirty = true;
                                    }
                                    ui.label("W");
                                    if ui
                                        .add(egui::DragValue::new(&mut q[3]).speed(0.02))
                                        .changed()
                                    {
                                        self.scene_dirty = true;
                                    }
    ...
    fn draw_animation_toolbar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label("Speed:");
        ui.add(
            egui::DragValue::new(&mut self.animation_editor_state.playback_speed)
                .speed(0.1)
                .clamp_range(0.1..=3.0)
                .suffix("x"),
        );
        ...
        if ui
            .button("➕ Add Keyframe")
            .on_hover_text("Add keyframe at current time for selected bones")
            .clicked()
        {
        ...
        if ui
            .button("🗑 Delete")
            .on_hover_text("Delete selected keyframes")
            .clicked()
        {
            if let Some(entity_id) = self.selected_entity_id {
                if let Some(scene) = &mut self.current_scene {
                    if let Some(entity) = scene.entities.iter_mut().find(|e| e.id == entity_id) {
                        if let Some(config) = &mut entity.animator_config {
                            let mut kf_indices =
                                self.animation_editor_state.selected_keyframes.clone();
    ...
    fn draw_animation_timeline(&mut self, ui: &mut egui::Ui) {
        egui::Frame::dark_canvas(ui.style()).show(ui, |ui| {
            ...
            // Draw time markers
            let step = if state.timeline_zoom > 2.0 {
                0.25
            } else if state.timeline_zoom > 1.0 {
                0.5
            } else {
                1.0
            };
            ...
            for i in 0..num_tracks.min(20) {
                ...
                painter.line_segment(
                    [
                        egui::pos2(rect.min.x + time_offset - 5.0, y),
                        egui::pos2(rect.min.x + time_offset - 5.0, y + track_height),
                    ],
                        ...
                        .and_then(|id| scene.entities.iter().find(|e| e.id == id))
                    {
                        if let Some(config) = &entity.animator_config {
                            for (kf_idx, kf) in config.editor_keyframes.iter().enumerate() {
                                if kf.bone_index == i {
                                    let kf_x =
                                        rect.min.x + time_offset + kf.time * pixels_per_second;
                                    if kf_x >= rect.min.x + time_offset && kf_x <= rect.max.x {
                                        let is_kf_selected =
                                            state.selected_keyframes.contains(&kf_idx);
                    ...
                    if let Some(config) = &entity.animator_config {
                        if config.clip_names.is_empty() {
                            ...
                            if config.bone_names.is_empty() {
                                for i in 0..16 {
                                    let selected =
                                        self.animation_editor_state.selected_bones.contains(&i);
                                    ...
                                    if ui.selectable_label(selected, label).clicked() {
                                        if selected {
                                            ...
                                            self.animation_editor_state.selected_bones.push(i);
                                        }
                                    }
                                }
                            } else {
                                for (i, name) in config.bone_names.iter().enumerate() {
                                    ...
                                    while let Some(parent_idx) = curr {
                                        depth += 1;
                                        curr =
                                            config.bone_parents.get(parent_idx).copied().flatten();
                                        if depth > 10 {
                                            break;
                                        } // Safety
                                    }
    ...
    fn draw_animation_preview(&mut self, ui: &mut egui::Ui) {
        ...
        if self.animation_editor_state.is_playing {
            let now = std::time::Instant::now();
            let dt = now
                .duration_since(self.animation_editor_state.last_update)
                .as_secs_f32();
            self.animation_editor_state.last_update = now;

            self.animation_editor_state.current_time +=
                dt * self.animation_editor_state.playback_speed;
        ...
        let clip_info = self
            ...
            .and_then(|e| e.animator_config.as_ref())
            .and_then(|c| {
                self.animation_editor_state.selected_clip_idx.and_then(|i| {
                    c.clip_names.get(i).map(|name| {
                        (
                            name.clone(),
                            c.clip_durations.get(i).copied().unwrap_or(0.0),
                        )
                    })
                })
            })
        ...
        painter.text(
            ...
            format!(
                "🎬 Clip: {} | ⏱ Time: {:.2}s",
                clip_info, self.animation_editor_state.current_time
            ),
        ...
        if self.push_connected {
            if let Some(id) = self.selected_entity_id {
                if let Some(clip_idx) = self.animation_editor_state.selected_clip_idx {
                    let _ =
                        self.command_tx
                            .send(AppCommand::Send(SceneUpdate::PreviewAnimationAt {
                                id,
                                clip_index: clip_idx,
...
impl eframe::App for EditorApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok(ev) = self.event_rx.try_recv() {
            event_received = true;
            match ev {
                AppEvent::QuestDetected(device) => {
                    self.quest_device = device;
                    if let Some(device) = &self.quest_device {
                        if !self.push_connected {
                            self.status = format!("Quest detected: {}", device.display_name());
                        }
                    } else if !self.push_connected {
                        self.status = "No Quest connection found for push".into();
                    }
                }
                AppEvent::Connected => {
                    self.status = "Push connected".into();
                    self.push_connected = true;
                }
                AppEvent::Disconnected => {
                    self.status = "Push disconnected".into();
        ...
        egui::TopBottomPanel::top("menu").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                        ...
                        ui.menu_button("📋 Recent Scenes", |ui| {
                            ...
                            for path in &recent_clone {
                                ...
                                if ui.button(display_name).clicked() {
                                    ...
                                    if self.check_unsaved_and_set_action(PendingAction::OpenScene(
                                        path_clone.clone(),
                                    )) {
                                    ...
                                    match stfsc_engine::project::Project::load(path) {
                                        Ok(proj) => {
                                            self.recent_projects
                                                .add(&proj.metadata.name, &recent.path);
                                            let _ = self.recent_projects.save();
                                            self.apply_loaded_project(ctx, proj);
                                        }
                                        Err(e) => {
                    ...
                    if let Some(proj) = &mut self.current_project {
                        ...
                        if ui.button("📦 Export Project...").clicked() {
                            ...
                            self.export_output_dir =
                                proj.root_path.join("exports").to_string_lossy().to_string();
                ...
                ui.menu_button("Build", |ui| {
                    ...
                    if ui
                        .add_enabled(has_project, egui::Button::new("📦 Export Project..."))
                        .clicked()
                    {
                        if let Some(proj) = &self.current_project {
                            self.export_output_dir =
                ...
                ui.menu_button("View", |ui| {
                    ...
                    if self.render_quality == RenderQuality::High {
                        ui.add(
                            egui::Slider::new(&mut self.resolution_scale, 0.1..=1.0)
                                .text("Resolution Scale"),
                        );
                        ui.add(
                            egui::Slider::new(&mut self.sharpening_amount, 0.0..=1.0)
                                .text("Sharpening"),
                        );
                    }
                ...
                ui.menu_button("Scene", |ui| {
                    if ui
                        .add_enabled(
                            self.push_connected,
                            egui::Button::new("Push Scene to Live Runtime"),
                        )
                        .clicked()
                    {
                        self.push_scene_to_live_runtime();
                        self.status = "Pushed scene to live runtime".into();
                        ui.close_menu();
                    }
                    if ui
                        .add_enabled(self.push_connected, egui::Button::new("Clear Live Runtime"))
                        .clicked()
                    {
                        let _ = self
                            .command_tx
                    ...
                    if let Some(scene) = &mut self.current_scene {
                        ...
                        if ui
                            .checkbox(&mut scene.respawn_enabled, "Enable Min Y Respawn")
                            .changed()
                        {
                            changed = true;
                        }
                        ui.horizontal(|ui| {
                            ui.label("Min Y:");
                            if ui
                                .add(egui::DragValue::new(&mut scene.respawn_y).speed(0.1))
                                .changed()
                            {
                                changed = true;
                            }
                        ...
                        if changed && self.push_connected {
                            let _ = self.command_tx.send(AppCommand::Send(
                                SceneUpdate::SetRespawnSettings {
                                    enabled: scene.respawn_enabled,
                                    y_threshold: scene.respawn_y,
                ...
                ui.separator();
                let play_label = if self.play_mode { "Stop" } else { "Play" };
                let play_color = if self.play_mode {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let c = if self.push_connected {
                        egui::Color32::GREEN
                    } else if self.quest_device.is_some() {
                        egui::Color32::YELLOW
                    } else {
                        egui::Color32::GRAY
                    };
        ...
        egui::SidePanel::left("project")
            ...
            .show(ctx, |ui| {
                ...
                egui::CollapsingHeader::new("Player / Push")
                    .default_open(true)
                    .show(ui, |ui| {
                        let player_mode =
                            if self.viewport_graphics.is_some() && self.use_gpu_viewport {
                                "Embedded Player: GPU"
                            } else {
                                "Embedded Player: Software"
                            };
                        ui.label(player_mode);
                        ui.separator();
                        ui.horizontal(|ui| {
                            ui.label("Quest:");
                            let quest_label = self
                                .quest_device
                                .as_ref()
                                .map(|device| device.display_name())
                                .unwrap_or_else(|| "Not detected".into());
                            ui.label(quest_label);
                            if ui.button("Check Quest Connection for Push").clicked() {
                                let _ = self.command_tx.send(AppCommand::DetectQuest);
                            }
                        });
                        ui.horizontal(|ui| {
                            ui.add(
                                egui::TextEdit::singleline(&mut self.push_addr)
                                    .desired_width(110.0),
                            );
                            if self.push_connected {
                                if ui.button("Disconnect").clicked() {
                                    let _ = self.command_tx.send(AppCommand::DisconnectPush);
                                }
                            } else if self.quest_device.is_some() {
                                if ui.button("Connect Quest Push").clicked() {
                                    if let Some(device) = self.quest_device.clone() {
                                        let _ = self
                                            .command_tx
                                            .send(AppCommand::ConnectQuestPush { device });
                                    }
                                }
                            } else if ui
                                .button("Connect External Runtime Push")
                                .on_hover_text("Only use this when a separate runtime is already listening on the address")
                                .clicked()
                            {
                                let _ = self.command_tx.send(AppCommand::ConnectDebugPush {
            ...
            if let Some(id) = self.selected_entity_id {
                if let Some(scene) = &mut self.current_scene {
                    ...
                    if let Some(entity) = scene.entities.iter_mut().find(|e| e.id == id) {
                            ...
                            ui.vertical(|ui| {
...

                                {
                                    let selected_slot = self
                                        .selected_script_slot_by_entity
                                        .entry(entity_id_for_scripts)
                                        .or_insert(0);
                                    ...
                                    for script_index in 0..components.len() {
                                        ui.horizontal(|ui| {
                                            ...
                                            egui::ComboBox::from_id_source(format!(
                                                "script_name_{}_{}",
                                                entity_id_for_scripts, script_index
                                            ))
                                    ...
                                    if components.len() < MAX_SCRIPTS_PER_ENTITY {
                                        ui.menu_button("+ Add Component", |ui| {
                                            for script_name in BUILTIN_SCRIPT_NAMES {
                                                if ui.button(*script_name).clicked() {
                                                    if components.len() < MAX_SCRIPTS_PER_ENTITY {
                                                        components.push(ScriptComponent::builtin(
                                                            *script_name,
                                                        ));
                                                        *selected_slot = components.len() - 1;
                                                        scripts_changed = true;
                                    ...
                                    ui.collapsing("Edit Script Source", |ui| {
                                        if components.is_empty() {
                                            ui.label("Add a script component to edit its source.");
                                        } else {
                                            ...
                                            ui.horizontal(|ui| {
                                                ui.label("Editing:");
                                                egui::ComboBox::from_id_source(format!(
                                                    "script_source_picker_{}",
                                                    entity_id_for_scripts
                                                ))
                                            ...
                                            ui.horizontal(|ui| {
                                                ui.label("Name:");
                                                if ui
                                                    .text_edit_singleline(&mut component.name)
                                                    .changed()
                                                {
                                                    scripts_changed = true;
                                                }
                                            ...
                                            ui.horizontal(|ui| {
                                                if ui
                                                    .checkbox(&mut component.enabled, "Enabled")
                                                    .changed()
                                                {
                                                    script_source_changed = true;
                                                }
                                                egui::ComboBox::from_id_source(format!(
                                                    "script_compile_mode_{}_{}",
                                                    entity_id_for_scripts, script_index
                                                ))
                                            ...
                                            let edit =
                                                egui::TextEdit::multiline(&mut component.source)
                                                    .desired_rows(10)
                                                    .code_editor()
                        ...
                        if let Some(anim_config) = &mut entity.animator_config {
                            egui::CollapsingHeader::new("🎬 Animator Controller")
                                ...
                                .show(ui, |ui| {
                                    ...
                                    if ui.button("Push Animator").clicked() {
                                        if self.push_connected {
                        ...
                        if inspector_dirty && self.push_connected {
                            let _ = command_tx.send(AppCommand::Send(SceneUpdate::UpdateMaterial {
                                id: entity.id,
            ...
            egui::Window::new("New Project")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        {
                            let root = std::path::PathBuf::from(&self.new_project_dir)
                                .join(&self.new_project_name);
                            let mut proj = stfsc_engine::project::Project::new(
            ...
            if let Some(ref rx) = self.export_receiver {
                while let Ok(msg) = rx.try_recv() {
                    match msg {
                        ...
                        ExportMessage::Complete {
                            platform,
                            output_path,
                        } => {
                            self.export_log.push_str(&format!(
                                "✅ {} export complete: {}\n",
                    ...
                    if let Some(proj) = &self.current_project {
                        ...
                        match self.export_opt_level {
                            ...
                            stfsc_engine::project::OptLevel::Release => {
                                ui.label(
                                    "⚖️ Recommended - good optimization, reasonable build time",
                                );
                            }
                        ...
                        ui.horizontal(|ui| {
                            let can_export = !self.export_in_progress
                                && (self.export_target_linux
                                    || self.export_target_windows
                                    || self.export_target_quest3
                                    || self.export_target_quest_pro);
                                ...
                                .clicked()
                            {
                                ...
                                std::thread::spawn(move || {
                                    let exporter =
                                        stfsc_engine::project::export::ProjectExporter::new(
                                            engine_root,
                                            output_dir,
                                        );
                        ...
                        .show(ui, |ui| {
                            ...
                            for file in &files_clone {
                                let display_name = std::path::Path::new(file)
                                    ...
                                    .unwrap_or(file);
                                if ui
                                    .selectable_label(false, format!("📄 {}", display_name))
                                    .clicked()
                                {
            ...
            egui::Window::new("🖼 UI Editor")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.separator();
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.button("Push to Live Runtime").clicked() && self.push_connected {
                                if let Some(scene) = &self.current_scene {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.vertical(|ui| {
                            ...
                            let rect = response.rect;
                            if response.drag_started() {
                                self.ui_drag_last_delta = egui::Vec2::ZERO;
                            }
                            ...
                            if response.dragged() && self.ui_drag_start_element.is_some() {
                                if let (Some(etype), Some(id), Some(scene)) = (&self.selected_ui_element_type, self.selected_ui_id, &mut self.current_scene) {
                                    if let Some(nl) = scene.ui_layouts.get_mut(self.selected_layout_idx) {
                                        let total_delta = response.drag_delta() / scale;
                                        let delta = total_delta - self.ui_drag_last_delta;
                                        self.ui_drag_last_delta = total_delta;
                                        match etype.as_str() {
        ...
        let needs_repaint = self.play_mode
            || self.animation_editor_state.is_playing 
