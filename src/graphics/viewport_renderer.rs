...
use super::{GlobalData, GraphicsContext};
...
impl ViewportRenderer {
    ...
    pub fn new(graphics: Arc<GraphicsContext>, width: u32, height: u32) -> Result<Self> {
        ...
        let motion_view = Self::create_image_view(
            device,
            motion_image,
            motion_format,
            vk::ImageAspectFlags::COLOR,
        ...
        let global_descriptor_set = graphics
        ...
        let default_material_ds =
            if let Ok((albedo, normal, mr)) = graphics.create_default_pbr_textures() {
                graphics
                    .create_material_descriptor_set(&albedo, &normal, &mr)
                    .ok()
            } else {
                None
            };
        ...
        let post_color_view = Self::create_image_view(
            device,
            post_color_image,
            color_format,
            vk::ImageAspectFlags::COLOR,
        ...
        let post_color_ref = [vk::AttachmentReference::builder()
            .attachment(0)
        ...
        let post_render_pass = unsafe {
            device.create_render_pass(
                &vk::RenderPassCreateInfo::builder()
                    .attachments(&post_attachments)
                    .subpasses(&post_subpass),
                None,
            )?
        };

        let post_fb_attachments = [post_color_view];
        let post_framebuffer = unsafe {
            device.create_framebuffer(
                &vk::FramebufferCreateInfo::builder()
                    .render_pass(post_render_pass)
                    .attachments(&post_fb_attachments)
                    .width(width)
                    .height(height)
                    .layers(1),
                None,
        ...
        let post_bindings = [vk::DescriptorSetLayoutBinding::builder()
            .binding(0)
            .descriptor_type(vk::DescriptorType::COMBINED_IMAGE_SAMPLER)
            .descriptor_count(1)
            .stage_flags(vk::ShaderStageFlags::FRAGMENT)
            .build()];
        let post_ds_layout = unsafe {
            device.create_descriptor_set_layout(
                &vk::DescriptorSetLayoutCreateInfo::builder().bindings(&post_bindings),
                None,
            )?
        };
        ...
        let post_descriptor_pool = unsafe {
        ...
        let post_descriptor_sets_vec = unsafe {
        ...
        // 16. Post Pipeline
        let post_push_range = [vk::PushConstantRange::builder()
            .stage_flags(vk::ShaderStageFlags::FRAGMENT)
            .offset(0)
            .size(4)
            .build()];
        let post_pipeline_layout = unsafe {
            device.create_pipeline_layout(
                &vk::PipelineLayoutCreateInfo::builder()
                    .set_layouts(&[post_ds_layout])
                    .push_constant_ranges(&post_push_range),
                None,
            )?
        };
        ...
        let blend_attachment = [vk::PipelineColorBlendAttachmentState::builder()
            ...
            .build()];
        let color_blend =
            vk::PipelineColorBlendStateCreateInfo::builder().attachments(&blend_attachment);
    ...
    pub fn upload_mesh_with_material(
        &mut self,
        mesh: &Mesh,
        material_ds: Option<vk::DescriptorSet>,
    ) -> Result<ViewportMeshHandle> {
    ...
    pub fn resize(&mut self, new_width: u32, new_height: u32) -> Result<()> {
        ...
        unsafe {
            self.graphics.device.device_wait_idle()?;
        } // Wait for all frames to finish before resizing
    ...
    pub fn render_and_readback(&mut self) -> Result<&[Color32]> {
        ...
        unsafe {
            *self.global_mapped = GlobalData {
                ...
                camera_pos: [
                    self.render_params.camera_pos.x,
                    self.render_params.camera_pos.y,
                    self.render_params.camera_pos.z,
                    self.render_params.ambient,
                ],
                light_dir: [
                    self.render_params.light_dir.x,
                    self.render_params.light_dir.y,
                    self.render_params.light_dir.z,
                    0.0,
                ],
            };
        }
            ...
            if let Some(global_ds) = self.global_descriptor_set {
                ...
                for instance in &self.pending_instances {
                    if let Some(mesh_data) = self.mesh_cache.get(&instance.handle) {
                        ...
                        let target_pipeline = if is_skinned {
                            self.graphics.skinned_pipeline
                        } else {
                            self.graphics.pipeline
                        };
                        let target_layout = if is_skinned {
                            self.graphics.skinned_pipeline_layout
                        } else {
                    ...
                    instance_idx += 1;
                    if instance_idx as usize >= self.max_instances {
                        break;
                    }
                }
            }
        ...
        unsafe {
            ...
            device.cmd_bind_descriptor_sets(
                ...
                0,
                &[self.post_descriptor_sets[self.current_frame]],
                &[],
            );
            ...
            device.cmd_push_constants(
                ...
                0,
                std::slice::from_raw_parts(&self.sharpening_amount as *const f32 as *const u8, 4),
            );
